...
class LoadMoreFollowerEpic(private val userRepository: UserRepository,
    ...
    override fun apply(action: PublishSubject<Action>, state: BehaviorSubject<FollowState>): Observable<LoadMoreResult> {
        return action.ofType(FollowAction.LoadMoreFollower::class.java)
                .flatMap {
                    val page = state.value.page + 1
                    userRepository.getFollowers(it.id, USER_LIMIT, page)
                            .map {
